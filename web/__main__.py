...
"""웹 서버 CLI.
    ...
    python3 -m web --demo                          # 체험(/try) — 샘플 4주 · 방문자별 샌드박스 · 미리 잰 결과
    python3 -m web --workspace ~/맞장부            # 설치된 PC 의 로컬 UI — 내 자료
    python3 -m web --demo --host 127.0.0.1 --port 8108 --cors-origin https://stoporder.github.io

기본 주소 http://127.0.0.1:8108/ — `/` 랜딩 · `/try/` 앱 · `/download/` · `/phone/`.
모델 서버(llama-server)는 따로 띄운다: `python3 -m engine serve --model <gguf>` 가 명령을 알려준다.
"""
...
from engine.model import DEFAULT_URL, health  # noqa: E402
from web.server import Config, make_server  # noqa: E402
